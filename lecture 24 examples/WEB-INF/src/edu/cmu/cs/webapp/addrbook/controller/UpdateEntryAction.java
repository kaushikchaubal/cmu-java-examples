...
public class UpdateEntryAction extends Action {
    ...
    private String createCase(HttpServletRequest request, EntryForm form) throws DAOException {
		// Create case
		...
		// Use newEntry because it has the new id
    	logCreate(request,newEntry);
		request.setAttribute("numEntries",entryDAO.getCount());  // It's now one bigger
		request.setAttribute("entry",newEntry);
    	request.setAttribute("message","Created record for "+newEntry.getLastName()+", "+newEntry.getFirstNames());
    	return "display-entry.jsp";
    }
    
    private String updateCase(HttpServletRequest request, EntryForm form) throws DAOException {
    	// Update case
    	Entry newEntry = new Entry(form.getIdNum());
    	copyFormPropsToDataBean(form,newEntry);
    	newEntry.setUpdated(new NMDate());
    	...
    	try {
    		...
    		Entry dbEntry = entryDAO.lookup(form.getIdNum());
    ...
    private void logChanges(HttpServletRequest request, Entry oldEntry, Entry newEntry) throws DAOException {
    ...
    private void logCreate(HttpServletRequest request, Entry newEntry) throws DAOException {
       	logDAO.write(request,"update-entry: creating new entry: id="+newEntry.getId());
       	Entry dummyOldEntry = new Entry(newEntry.getId());
       	logChanges(request,dummyOldEntry,newEntry);
    }
    
    
    private void logUpdate(HttpServletRequest request, Entry oldEntry, Entry newEntry) throws DAOException {
       	logDAO.write(request,"update-entry: updating entry: id="+newEntry.getId());
       	logChanges(request,oldEntry,newEntry);
    }

    private void logIfChanged(HttpServletRequest request, String fieldName, NMSQLDate oldV, NMSQLDate newV)
    	throws DAOException
    {
